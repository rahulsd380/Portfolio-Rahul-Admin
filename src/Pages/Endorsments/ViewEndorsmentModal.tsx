...
interface ViewEndorsmentModalProps {
  isModalOpen: boolean;
  ...
  return (
    ...
    >
      <div
        ...
        className={`absolute w-[600px] h-auto  text-white drop-shadow-2xl ${isModalOpen ? "opacity-1 translate-y-0 duration-300" : "-translate-y-20 opacity-0 duration-150"}`}
      >
          ...
          <div className="flex justify-between items-center mt-4">
            <div className="flex items-center justify-center gap-3">
              {[...Array(rating)].map((_, index) => (
                <img key={index} src={ICONS.star} alt="star" className="size-6" />
              ))}
            </div>
